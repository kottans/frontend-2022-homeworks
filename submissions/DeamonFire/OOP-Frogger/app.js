...
const MASSAGE = {
    gameOver: "Співчуваю, жуки пустили вас на добрива =(\nПотрібно їм помститись! \n\n\Запускаємо по новій?",
    victory: "Вітаю, ви успішно влаштували жукам потоп! Амінь! \nНо що ж це? На вільне місце прийшли жуки зі сходу! \n\nАллах Акбар невірним!!!\nЗапускаємо по новій!"
};
...
class Enemy extends Character{
    ...
    startPosition() {
        if(this.positionX > POSITION_ENEMY.end) {
            ...
            this.positionY = randomizer( Math.min(...COUNT_LINES) , Math.max(...COUNT_LINES) ) * GAME_CELL.cellY;
        }
    }
...
class Player extends Character{
    ...
    redrawStyle() {
        document.getElementsByTagName('body')[0].style.backgroundColor = `rgb(${this.red},${this.green},${this.blue})`;
        yourScore.innerHTML = `<span>Your score = ${this.score}</span>`;
        document.getElementsByTagName('span')[0].style.color = `rgb(${this.redScore},${this.greenScore},${this.blueScore})`;
...
// --------------------------------------------------------New object------------------------------------------------------
const allEnemies = [
    new Enemy(POSITION_ENEMY.firstStart, LINES.line1, randomizer( SPEED.min, SPEED.max ) , SPRETES.enemy),
    new Enemy(POSITION_ENEMY.firstStart, LINES.line2, randomizer( SPEED.min, SPEED.max ), SPRETES.enemy),
    new Enemy(POSITION_ENEMY.firstStart, LINES.line3, randomizer( SPEED.min, SPEED.max ), SPRETES.enemy),
    new Enemy(POSITION_ENEMY.firstStart, LINES.randomLine, randomizer( SPEED.min, SPEED.max ), SPRETES.enemy)
];
...
document.getElementsByTagName('body')[0].style.backgroundColor = `rgb(${BG_COLOR.red},${BG_COLOR.green},${BG_COLOR.blue})`;
document.getElementsByTagName('span')[0].style.color = `rgb(${COLOR.red},${COLOR.green},${COLOR.blue})`;
document.getElementsByTagName('span')[0].style.fontSize = '20px';
