...
function initMusicApp() {
  const tabs = document.querySelectorAll('.wrapper__item'), contentTitle = document.querySelector('.music__title'), contentDescr = document.querySelector('.music__descr'), parentItems = document.querySelector('.wrapper__list')
  ...
  parentItems.addEventListener('click', (event) => {
    ...
    if (target && target.classList.contains('wrapper__item')) {
        tabs.forEach((item, i) => {
      if (target == item) {
          contentTitle.innerText = musicStyle[i].title
          contentDescr.innerText = musicStyle[i].desc
        }
      })
    }
  })
