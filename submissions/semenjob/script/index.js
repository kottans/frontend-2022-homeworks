window.addEventListener("DOMContentLoaded", () => {
  ...
  const createStyleList = (title) => {
    return `<div class="wrapper__item">${title}</div>`;
  };

  const element = musicStyle.map(({ title }) => createStyleList(title));
  parentItems.innerHTML = element.join("");
  ...
  parentItems.addEventListener("click", function (event) {
    ...
    if (target.classList.contains("wrapper__item")) {
      ...
      const style = musicStyle.find((item) => {
        return item.title === targetItem;
      });
      contentDescr.innerText = style.desc;
      contentTitle.innerText = style.title;
    }
  });
});
