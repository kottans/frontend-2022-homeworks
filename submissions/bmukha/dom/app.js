...
const renderHero = (hero) => {
  ...
  return `<h2 class="hero-name">${heroName}</h2>
    ...
    <div class="hero-power">
        <img class="hero-power_img" src="./img/${heroClass}-heropower.webp" alt="heropower" />
    </div>
    <p class="lore">
        ${heroLore}
    </p>`;
...
document.querySelector('.menu').addEventListener('click', (event) => {
  const hero = event.target.parentElement.id;
  const info = heroes.find(
    (item) => item.heroClass.toLowerCase().split(' ').join('') === hero
  );
  content.innerHTML = renderHero(info);
});
